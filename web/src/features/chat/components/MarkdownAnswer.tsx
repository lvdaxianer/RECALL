...
export function MarkdownAnswer({ content, isStreaming, progressText }: MarkdownAnswerProps) {
  ...
  return trimmedContent || !isStreaming ? (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xs transition-shadow hover:shadow-sm">
      <StreamingIndicator isStreaming={isStreaming} progressText={progressText} />
      <MarkdownBody content={content} trimmedContent={trimmedContent} />
    </div>
  ) : undefined;
}
 ...
 * 流式回答进度提示。
 *
 * @param props - isStreaming 控制展示，progressText 为提示文案
 * @returns 流式提示 UI；非流式时不渲染
 * @author lvdaxianerplus
 * @date 2026-06-06
 */
function StreamingIndicator({ isStreaming, progressText }: { isStreaming: boolean; progressText?: string }) {
  return isStreaming ? (
 ...
 * Markdown 正文渲染区。
 *
 * @param props - content 为原文，trimmedContent 用于判断空内容
 * @returns Markdown 正文 UI；空内容时不渲染
 * @author lvdaxianerplus
 * @date 2026-06-06
 */
function MarkdownBody({ content, trimmedContent }: { content: string; trimmedContent: string }) {
...
const MARKDOWN_COMPONENTS = {
  ...
  img: ({ alt, src }: { alt?: string; src?: string }) => (
    <img alt={alt ?? DEFAULT_IMAGE_ALT} loading="lazy" src={src ?? DEFAULT_IMAGE_SRC} />
  ),
};
