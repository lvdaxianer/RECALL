...
async def insert_one(client: httpx.AsyncClient, item: dict) -> dict:
    """插入单条数据"""
    user_id = "test_user"
    payload = {
    ...
    response = await client.post(
        f"{BASE_URL}/api/v1/rag/{user_id}/insert",
        json=payload,
        timeout=60.0
    )
    ...
    return {
        'id': item['id'],
        'status': 'success' if result.get('code') == 200 else 'error',
        'response': result
    }
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        for i, item in enumerate(TEST_DATA, 1):
            try:
                result = await insert_one(client, item)
                if result['status'] == 'success':
                    success_count += 1
                    data = result['response'].get('data', {})
                    features = data.get('features', {})
                    category = features.get('category', 'N/A')
                    tags = features.get('tags', [])[:3]
                    print(f"[{i:3d}] {item['id']}: {category:10s} | tags={tags}")
                else:
                    fail_count += 1
                    print(f"[FAIL] {item['id']}: {result['response'].get('message', 'unknown error')[:50]}")
            except Exception as e:
                fail_count += 1
                print(f"[ERR] {item['id']}: {str(e)[:60]}")
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{BASE_URL}/api/v1/rag/test_user/search",
            json=search_payload,
            timeout=60.0
        )
        result = response.json()
        if result.get('code') == 200:
            for i, item in enumerate(result.get('data', []), 1):
                print(f"  [{i}] {item.get('id')}: score={item.get('score', 0):.4f}")
                print(f"      desc={item.get('description', '')[:40]}")
